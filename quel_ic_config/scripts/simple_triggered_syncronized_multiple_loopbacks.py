...
def simple_trigger_2(pg0: PulseGen, pgs_: Collection[PulseGen], cps_: Collection[PulseCap], num_wait_word1: int = 0):
    if pg0 not in pgs_:
        raise ValueError("pg0 is not a member of pgs_")

    for pg in pgs_:
        pg.num_wait_words = (pg.num_wait_words[0], num_wait_word1)
        pg.init_wave()
    ...
    # Notes: this is just a WorkAround!!
    awgs = {pg.awg for pg in pgs_}
    pg0.box.wss.start_emission(awgs)
...
if __name__ == "__main__":
    ...
    results = simple_trigger_2(
        pgs["SENDER0"],
        {pgs["SENDER0"], pgs["SENDER1"]},
        {cps["CAPTURER0"], cps["CAPTURER1"], cps["CAPTURER2"], cps["CAPTURER3"]},
        20,
    )

...
import pytest
from quel_inst_tool import E440xb, ExpectedSpectrumPeaks, MeasuredSpectrumPeak

from quel_ic_config_utils.simple_box import Quel1BoxType, Quel1ConfigOption, SimpleBoxIntrinsic, init_box_with_linkup
from testlibs.e440xb_helper import init_e440xb, measure_floor_noise
...
TEST_SETTINGS = (
    {
        ...
        "spa_type": "E4405B",
        "spectrum_image_path": "./artifacts/spectrum-074",
        "relative_loss": 0,
    },
    {
        "box_config": {
...
def fixtures(request):
    ...
    linkup0, linkup1, _, _, _, box = init_box_with_linkup(**param0["box_config"], refer_by_port=False)
    assert linkup0
    assert linkup1
    ...
    yield box, e440xb, make_outdir(request.param), request.param["port_availability"], request.param["relative_loss"]
...
def test_all_single_awgs(
    ...
    box.easy_stop(mxfe, line, control_port_rfswtich=not via_monitor, control_monitor_rfswtich=via_monitor)

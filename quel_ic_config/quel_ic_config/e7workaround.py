...
class E7LibBranch(Enum):
    SIMPLEMULTI_CLASSIC = 0
    SIMPLEMULTI_STANDARD = 1
    FEEDBACK = 2
...
_VERSION_TO_FWTYPE: Final[Mapping[Tuple[str, str, str], Tuple[E7FwType, E7FwLifeStage]]] = {
    ("a:2024/01/25-1", "a:2024/01/25-2", "*"): (E7FwType.SIMPLEMULTI_STANDARD, E7FwLifeStage.SUPPORTED),
...
def resolve_hw_type(hw_versions: Tuple[str, str, str]) -> Tuple[E7FwType, E7FwLifeStage]:
    for v, (ht, ls) in _VERSION_TO_FWTYPE.items():
...
def detect_branch_of_library() -> E7LibBranch:
    ...
    # Notes: Oops, the extension of SIMPLEMULTI firmware induces another confusion! I'll kill them all!
    ...
    if num_capmod == 2 and not has_sequencerctrl:
        return E7LibBranch.SIMPLEMULTI_CLASSIC
    elif num_capmod == 4 and not has_sequencerctrl:
        return E7LibBranch.SIMPLEMULTI_STANDARD

...
![QuEL-1 SE 前面パネル](images/quel1se_riken8_front.jpeg)
...
![QuEL-1 SE 背面パネル](images/quel1se_riken8_back.jpeg)
...
この割り当ては、ポート#6からポート#9までの4つのポートの間で融通可能です。
リンクアップ時のオプション指定(`--config_options`)で、上の表で示したデフォルトの割り当て(`se8_mxfe1_awg1331`)の他に、2つのプリセットを用意してあります。
具体的には、SMAポート#6, #7, #8, #9のそれぞれに2つずつのAWGを割り当てるプリセット(`se8_mxfe1_awg2222`)、
SMAポート#6と#9のそれぞれに3つのAWG、SMAポート#7と#8のそれぞれに1つのAWGを割り当てるプリセット(`se8_mxfe1_awg3113`) を選択可能です。
プリセットはリンクアップ時に決定しなければなりません。
他のプリセットも可能ですので、 今後、ユーザ様のご要望に応じて調整したいと考えています。
なお、SMAポート#1, #2, #3 についても同様の調整が可能です。 
必要が生じた際には、お問い合わせください。

...
@pytest.mark.parametrize(
    ("mxfe", "awg_idx", "lo_mhz", "cnco_mhz", "fnco_mhz", "sideband"),
    [
        (0, 0, 11500, 1500, 0, "L"),
        (0, 0, 11500, 1500, 10, "L"),
        (0, 0, 11500, 1500, -150, "L"),
        (1, 0, 11500, 1500, 0, "L"),
        (1, 0, 11500, 1500, 100, "L"),
        (1, 0, 11500, 1500, -20, "L"),
    ],
)
def test_readin_loopback(
    mxfe: int, awg_idx: int, lo_mhz: int, cnco_mhz: int, fnco_mhz: int, sideband: str, css_p2_p3, outdir
):
    mxfe_g, mxfe_c = css_p2_p3[mxfe]
    mxfe_g.run(0, awg_idx, lo_mhz=lo_mhz, cnco_mhz=cnco_mhz, fnco_mhz=fnco_mhz, sideband=sideband)
    mxfe_c.run(
        "r",
        num_words=65536 * 16,
        active_units={0},
    ...
    plt.cla()
    plt.plot(f, p / len(x))
    plt.savefig(outdir / f"mxfe{mxfe:d}-line0-ch{awg_idx:d}-fnco{fnco_mhz:d}.png")

...
def quel1_test_linkup_body(args: argparse.Namespace) -> int:
    ...
    if args.background_noise_threshold is None:
        background_noise_threshold: float = LinkupFpgaMxfe._DEFAULT_BACKGROUND_NOISE_THRESHOLD_AT_RELINKUP
    else:
        background_noise_threshold = args.background_noise_threshold
...
def quel1_firmware_version_body(args: argparse.Namespace) -> int:
    ...
    print(f"e7awg firmware version: {box.wss.hw_version}")
    print(f"type of the firmware: {box.wss.hw_type}")
    print(f"life cycle of the firmware: {box.wss.hw_lifestage}")
    return 0
...
def quel1se_tempctrl_reset_body(args: argparse.Namespace) -> int:
    ...
    if not (hasattr(box.css, "start_tempctrl") and hasattr(box.css, "set_tempctrl_state_count")):

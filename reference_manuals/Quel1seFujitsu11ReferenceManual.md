# QuEL-1 SE 7-11GHz対応 FY2024モデル 取扱説明書
...
Type-Aの装置は、同一構成のポート群を2つ（#0〜#6, #7〜#13)持っており、それぞれの系統を1つの読み出し信号系に接続している4つの量子ビットに接続して使います。
...
なお、複数台の制御装置を同時にリンクアップする場合には `quel1_parallel_linkup` コマンドが便利です。

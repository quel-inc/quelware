...
これを実現するために、 Alveo U50のファームウェアを制御する低レイヤのライブラリを従来の [`e7awg_sw`](https://github.com/e-trees/e7awg_sw/tree/simple_multi_standard) 
を [`e7awghal`](https://github.com/quel-inc/quelware/tree/main/e7awghal) に挿し替え、WSS層をほぼ全面的に書き直した。
`e7awghal`パッケージ は、`e7awg_sw`パッケージの下半分と、`quel_clock_master`パッケージを統合したパッケージである。
...
WSSを操作するAPIは、おおっざぱに、初期化、情報参照、設定、波形生成・取得の4つに分類できる。
初期化と情報参照のAPIは、数が少ないので、まるまる置き換えても多寡がしれているので、先に残りの2つについて説明する。

### 設定系API
設定系のAPIは、0.8.xからの変更点は見かけ上は大したことがない。
0.8.x で試験的に提供している `Quel1BoxWithRawWss`の `config_channel()` が `e7awgsw.WaveSeq` を、
`config_runit()` が `e7awgsw.CapParam` をそれぞれ引数にとる形式を継承している。
パラメタのオブジェクトは、e7awghal が定義する対応物に変更しているが、内容についてシンプルな対応関係が付く。
そういう意味で、コードの変更はさほど大きくならないかもしれない。

しかし、パラメタの作成時に、波形データの扱いに関する基本的な発想が変更されているので注意が必要である。
0.8.x では波形生成の度にWaveChunkが握っているiqデータの配列を、制御装置にアップロードしてから波形生成していた。
この設計は、シンプルなことをする限りではうまく使えるが、将来を考えると心許ない。
たとえば、シーケンサが古典IFを実装する場合には、制御装置のメモリ上に複数の波形データを配置しておくべきだ。
...
そこで、波形発生開始APIがFutureオブジェクトを返す形に非同期化し、このオブジェクトの `cancel()` を叩くことで停止する作りになっている。
(注: 通常の`concurrent`モジュールの`Future`はスレッドの実行前に限り `cancel()` 可能という意味論だが、ここではそれを拡張して、波形生成を途中で停止できるような拡張をしている。）
このオブジェクトを `result()`すれば、波形発生が終了したことを保証できるので、従来のようにDoneフラグを明示的に確認する必要はない。

波形取得APIについては、上記第3項目の「波形生成と波形取得の同時開始」を1つのAPIで安全かつ簡単に行えるようになったのが、一番の目玉であると思う。
実用上は、第2項目の即時の波形取得は使い所が少ないと思う。
また、あまり使うことはないだろうが、波形取得も `cancel()`可能になった。
同時開始の設計意図は、キャプチャモジュールのトリガ設定の管理を自動化し、開始前の設定ミスの検出及び防止や、異常終了時やキャンセル時の整合的な復帰処理の実現にある。
このBox層のAPIは、WSS層のAPIを2つ組み合わせて実現しているが、異常時の対応を適切に行うには、ファームウェアの挙動の詳細を理解する必要がなり、かなり面倒なことになる。
...
全装置の時刻カウンタが数ミリ秒の精度で合っていれば、この値の2000(= 125MHz / 62.5MHz)の剰余を比較することで装置間の時刻のずれを8nsの精度で検出できるはずだった。
...
[GETTING_STARTED.md](./GETTING_STARTED.md#コード例)により詳しい説明がある。

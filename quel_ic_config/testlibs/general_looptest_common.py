...
class PulseGen:
    ...
    def __init__(
        ...
        if not box_status:
            raise RuntimeError(f"sender '{name}' is not available due to link problem of '{box}'")
        if wave_samples % self.NUM_SAMPLES_IN_WAVE_BLOCK != 0:
            raise ValueError(f"wave samples must be multiple of {self.NUM_SAMPLES_IN_WAVE_BLOCK}")
        ...
        self.line: int = line
        self.channel: int = 0
        self.awg: int = self.box.rmap.get_awg_of_channel(self.group, self.line, self.channel)
    ...
    def init_config(self) -> None:
        ...
        self.box.config_channel(group=self.group, line=self.line, channel=self.channel, fnco_freq=self.fnco_freq)
...
class PulseCap:
    ...
    def init(self):
        self.box.config_rline(group=self.group, rline=self.rline, lo_freq=self.lo_freq, cnco_freq=self.cnco_freq)
        self.box.config_runit(group=self.group, rline=self.rline, runit=self.runit, fnco_freq=self.fnco_freq)
    ...
    def capture_now(self, *, num_samples: int, delay: int = 0):
        thunk = self.box.wss.simple_capture_start(
            capmod=self.capmod, capunits=(self.runit,), num_words=num_samples // 4, delay=delay, triggering_awg=None
        )

...
- 排他的使用を支援するロック機構を実装。
  - QuEL-1以前の装置及び、QuEL-1 SE でConfigSubsystem(css)のファームウェアが v1.2.1以前の装置は、従来のファイルロックと類似の仕組みをBox単位で実装。
    - ロックファイル用のディレクトリ(/run/quelware)をNFSで共有することで、複数ホスト間でのロックを実現。
    - ロックの仕組みを持たない古いバージョンのソフトウェアからのアクセスを防ぐことができないので注意。
  - QuEL-1 SEで cssファームウェアが v1.3.0以降の装置は、装置本体のロック機構も利用可能。
    - cssについては、ロックを取得していないホスト側オブジェクトからのアクセスを遮断する。
    - wssについては、ロックの仕組みを持たない古いバージョンのソフトウェアからのアクセスを防ぐことができないので注意。
    - v1.3.0ファームウェアはまだ限定公開の段階。

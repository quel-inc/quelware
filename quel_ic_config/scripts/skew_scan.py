...
def skew_scanner():
    ...
    results = cp.capture_at_multiple_triggers_of(pg=pg0, num_iters=17, num_samples=1024, delay=0)
    schedule = boxpool.emit_at(
        cp=cp,
        pgs={pg0, pg1},
        min_time_offset=125_000_000,
        time_counts=[i * 12_500_000 + (i - 8) for i in range(17)],
    )
...
if __name__ == "__main__":
    ...
    iq_list, chunks_list, schedule = skew_scanner()
    plot_iqs({f"skew {idx-8}": iq for idx, iq in enumerate(iq_list)})

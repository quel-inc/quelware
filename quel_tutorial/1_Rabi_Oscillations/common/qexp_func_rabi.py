...
def frequency_calc_ctrl(
    ...
    cnco_min: int = 1_125_000_000,
    cnco_max: int = 1_875_000_000,
) -> tuple[int, int, int]:

    freq_cnco = 1_500_000_000
    if sideband == "L":
        # freq_target = freq_lo - (freq_awg + freq_nco)
        freq_lo = round((freq_target + freq_cnco)/lo_unit) * lo_unit
        if freq_lo < lo_min or freq_lo > lo_max:
            raise ValueError("LO frequency is out of range.")
        freq_cnco = round((freq_lo - freq_target)/nco_unit) * nco_unit
        if freq_cnco < cnco_min or freq_cnco > cnco_max:
            raise ValueError("CNCO frequency is out of range.")
...
def frequency_calc_ro(
    ...
    if sideband == "L":
        ...
        if freq_lo < lo_min or freq_lo > lo_max:
            raise ValueError("LO frequency is out of range.")
        freq_cnco = round((freq_lo - freq_target)/nco_unit) * nco_unit
        if freq_cnco < cnco or freq_cnco > cnco:
            raise ValueError("CNCO frequency is out of range.")
    elif sideband == "U":
        # freq_target = freq_lo + (freq_awg + freq_nco)
        freq_lo = round((freq_target - freq_cnco)/lo_unit) * lo_unit
...
def Rabi_oscillation_model(x, A, tau, f, phi, B):
...
def Rabi_fitting(
    ...
    param_init = [1.0, 100., 15.e-3, 0., 0.]
    param_bounds = ([0.9, 0., 0., -np.pi, -0.2], [1.1, 1.5e9, 40.e-3, np.pi, 0.2])
    
    popt, pcov = curve_fit(Rabi_oscillation_model, ctrl_pulse_width_list, q_norm, p0=param_init, bounds=param_bounds)
    fitted_q = Rabi_oscillation_model(ctrl_pulse_width_list, *popt)
...
def func(f_drive, f_rabi, f_reso, cons):
...
def plot_rabi_freq(
    ...
    popt, pcov = curve_fit(func, x_list, y_list, p0=parameter_initial, maxfev=100000)
    rabi_freq_fit = func(x_list, *popt)
...
def plot_Chevron_pattern(
        chevron_pattern: npt.NDArray[npt.NDArray[np.float64]],
        x_list : npt.NDArray[np.float64],
        y_list : npt.NDArray[np.float64],
        figsize: tuple[int, int] = (8, 4),
    ):
    fig, ax = plt.subplots(1, 1, figsize=figsize)
    cax = ax.imshow(chevron_pattern, aspect='auto', cmap='viridis', 
                extent=[x_list[0], x_list[-1], y_list[0], y_list[-1]], origin='lower')
...
def plot_fft_pattern(
        fft_pattern: npt.NDArray[npt.NDArray[np.float64]],
        x_list : npt.NDArray[np.float64],
        y_list : npt.NDArray[np.float64],
        figsize: tuple[int, int] = (8, 4),
    ):

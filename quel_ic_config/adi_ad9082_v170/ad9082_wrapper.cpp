...
int32_t adi_ad9082_jesd_rx_bit_rate_get(adi_ad9082_device_t *device,
                                        uint64_t *bit_rate_bps);
int32_t adi_ad9082_adc_ddc_fine_sync_next_set(adi_ad9082_device_t *device,
...
int ad9081_callback_spi_xfer(void *handle, uint8_t *in_data, uint8_t *out_data,
  ...
  return retcode ? API_CMS_ERROR_OK : API_CMS_ERROR_SPI_XFER;
}
...
int ad9081_callback_delay_us(void *handle, uint32_t us) {
  ...
  PyGILState_Release(state);
  return retcode ? API_CMS_ERROR_OK : API_CMS_ERROR_DELAY_US;
}
...
int ad9081_callback_log_write(void *handle, int32_t log_type,
  ...
  auto retcode = py::cast<bool>(result[0]);
  PyGILState_Release(state);
  return retcode ? API_CMS_ERROR_OK : API_CMS_ERROR_LOG_WRITE;
}
...
int ad9081_callback_reset_pin_ctrl(void *handle, uint8_t enable) {
  ...
  auto retcode = py::cast<bool>(result[0]);
  PyGILState_Release(state);
  return retcode ? API_CMS_ERROR_OK : API_CMS_ERROR_RESET_PIN_CTRL;
}
...
PYBIND11_MODULE(adi_ad9082_v170, m) {
  ...
  py::enum_<adi_cms_signal_type_e>(m, "CmsSignalType")
      .value("SIGNAL_CMOS", SIGNAL_CMOS, "CMOS signal")
      .value("SIGNAL_LVDS", SIGNAL_LVDS, "LVDS signal")
      .value("SIGNAL_CML", SIGNAL_CML, "CML signal")
      .value("SIGNAL_LVPECL", SIGNAL_LVPECL, "LVPECL signal")
      .value("SIGNAL_UNKNOWN", SIGNAL_UNKNOWN, "UNKNOWN signal")
      .export_values();

  py::enum_<adi_cms_signal_coupling_e>(m, "CmsSingalCoupling")
      .value("COUPLING_AC", COUPLING_AC, "AC coupled signal")
      .value("COUPLING_DC", COUPLING_DC, "DC signal")
      .value("COUPLING_UNKNOWN", COUPLING_UNKNOWN, "UNKNOWN coupling")
      .export_values();
  ...
  py::enum_<adi_cms_jesd_subclass_e>(m, "CmsJesdSubclass", py::arithmetic())
  ...
  py::class_<adi_ad9082_clk_t>(m, "Clk")
      .def(py::init([]() {
        auto self = std::make_unique<adi_ad9082_clk_t>();
        self->sysref_clk = nullptr;
        self->sysref_ctrl = nullptr;
        self->sysref_mode = SYSREF_NONE;
        return self;
      }))
  ...
  py::class_<adi_ad9082_device_t>(m, "Device")
      .def(py::init([]() {
        auto self = std::make_unique<adi_ad9082_device_t>();
        ...
        self->clk_info.sysref_clk = nullptr;
        self->clk_info.sysref_ctrl = nullptr;
        self->clk_info.sysref_mode = SYSREF_NONE;
        return self;
      }))
  ...
  m.def("hal_calc_tx_nco_ftw",
        [](adi_ad9082_device_t *device, uint64_t dac_freq, int64_t nco_shift,
           nco_ftw_t *nco_ftw) {
          nco_ftw->delta_b = 0;
          nco_ftw->modulus_a = 1;
          return adi_ad9082_hal_calc_tx_nco_ftw(device, dac_freq, nco_shift,
                                                &(nco_ftw->ftw));
        });
  m.def("hal_calc_rx_nco_ftw",
        [](adi_ad9082_device_t *device, uint64_t adc_freq, int64_t nco_shift,
           nco_ftw_t *nco_ftw) {
          nco_ftw->delta_b = 0;
          nco_ftw->modulus_a = 1;
          return adi_ad9082_hal_calc_rx_nco_ftw(device, adc_freq, nco_shift,
                                                &(nco_ftw->ftw));
        });
  ...
  m.def("device_chip_id_get", adi_ad9082_device_chip_id_get);
  m.def("device_clk_config_set", adi_ad9082_device_clk_config_set);
  m.def("device_clk_pll_lock_status_get", [](adi_ad9082_device_t *device) {
    uint8_t status = 0U;
    int32_t rc = adi_ad9082_device_clk_pll_lock_status_get(device, &status);
  ...
  m.def("dac_duc_nco_enable_set", adi_ad9082_dac_duc_nco_enable_set);
  m.def("dac_duc_nco_ftw_set", [](adi_ad9082_device_t *device, uint8_t dacs,
                                  uint8_t channels, nco_ftw_t *nco_ftw) {
    return adi_ad9082_dac_duc_nco_ftw_set(device, dacs, channels, nco_ftw->ftw,
  ...
  m.def("device_get_temperature",
        [](adi_ad9082_device_t *device, chip_temperatures_t *temps) {
          return adi_ad9082_device_get_temperature(device, &(temps->temp_max),
                                                   &(temps->temp_min));
        });
  ...
  py::enum_<adi_ad9082_adc_nco_mode_e>(m, "AdcNcoMode", py::arithmetic())
      .value("ADC_NCO_VIF", AD9082_ADC_NCO_VIF, "Variable IF Mode")
      .value("ADC_NCO_ZIF", AD9082_ADC_NCO_ZIF, "Zero IF Mode")
      .value("ADC_NCO_FS_4_IF", AD9082_ADC_NCO_FS_4_IF, "Fs/4 Hz IF Mode")
      .value("ADC_NCO_TEST", AD9082_ADC_NCO_TEST, "Test Mode")
      .export_values();

  m.def("adc_ddc_coarse_nco_mode_set", &adi_ad9082_adc_ddc_coarse_nco_mode_set);
  m.def("adc_ddc_coarse_nco_ftw_set", [](adi_ad9082_device_t *device,
                                         uint8_t cddcs, nco_ftw_t *nco_ftw) {
    return adi_ad9082_adc_ddc_coarse_nco_ftw_set(
        device, cddcs, nco_ftw->ftw, nco_ftw->modulus_a, nco_ftw->delta_b);
  });
  m.def("adc_ddc_coarse_nco_ftw_get",
        [](adi_ad9082_device_t *device, uint8_t cddc, nco_ftw_t *nco_ftw) {
          return adi_ad9082_adc_ddc_coarse_nco_ftw_get(
              device, cddc, &(nco_ftw->ftw), &(nco_ftw->modulus_a),
              &(nco_ftw->delta_b));
        });
  m.def("adc_ddc_fine_nco_mode_set", adi_ad9082_adc_ddc_fine_nco_mode_set);
  m.def("adc_ddc_fine_nco_ftw_set", [](adi_ad9082_device_t *device,
                                       uint8_t fddcs, nco_ftw_t *nco_ftw) {
    return adi_ad9082_adc_ddc_fine_nco_ftw_set(
        device, fddcs, nco_ftw->ftw, nco_ftw->modulus_a, nco_ftw->delta_b);
  });
  m.def("adc_ddc_fine_nco_ftw_get",
        [](adi_ad9082_device_t *device, uint8_t fddc, nco_ftw_t *nco_ftw) {
          return adi_ad9082_adc_ddc_fine_nco_ftw_get(
              device, fddc, &(nco_ftw->ftw), &(nco_ftw->modulus_a),
              &(nco_ftw->delta_b));
        });
  m.def("adc_ddc_coarse_select_set", adi_ad9082_adc_ddc_coarse_select_set);
  m.def("adc_ddc_fine_select_set", adi_ad9082_adc_ddc_fine_select_set);
  m.def("adc_ddc_coarse_sync_next_set",
        adi_ad9082_adc_ddc_coarse_sync_next_set);
  m.def("adc_ddc_coarse_sync_enable_set",
        adi_ad9082_adc_ddc_coarse_sync_enable_set);
  m.def("adc_ddc_coarse_trig_nco_reset_enable_set",
        adi_ad9082_adc_ddc_coarse_trig_nco_reset_enable_set);
  m.def("adc_ddc_fine_sync_next_set", adi_ad9082_adc_ddc_fine_sync_next_set);
  m.def("adc_ddc_fine_sync_enable_set",
        adi_ad9082_adc_ddc_fine_sync_enable_set);
  m.def("adc_ddc_fine_trig_nco_reset_enable_set",
        adi_ad9082_adc_ddc_fine_trig_nco_reset_enable_set);
  ...
  m.def("jesd_tx_lanes_xbar_set",
        [](adi_ad9082_device_t *device, adi_ad9082_jesd_link_select_e links,
           std::array<uint8_t, 8> &logical_lanes) {
          return adi_ad9082_jesd_tx_lanes_xbar_set(device, links,
                                                   logical_lanes.data());
        });
  ...
  m.def("jesd_tx_link_select_set", adi_ad9082_jesd_tx_link_select_set);
  ...
  m.def("jesd_rx_bit_rate_get", [](adi_ad9082_device_t *device) {
    uint64_t bit_rate_bps;
    int32_t rc = adi_ad9082_jesd_rx_bit_rate_get(device, &bit_rate_bps);
    return std::pair<int32_t, uint64_t>(rc, bit_rate_bps);
  });
  ...
  m.def("jesd_rx_ctle_manual_config_get",
  ...
  // Sync top-level
  m.def("sync_sysref_frequency_set",
        [](adi_ad9082_device_t *device, uint64_t dev_ref_freq_hz,
           uint64_t dac_freq_hz, uint64_t adc_freq_hz, uint8_t main_interp,
           uint8_t ch_interp, std::array<uint8_t, 4> &cddc_dcm,
           std::array<uint8_t, 8> &fddc_dcm,
           adi_ad9082_jesd_link_select_e jtx_links,
           adi_cms_jesd_param_t *jrx_param,
           std::array<adi_cms_jesd_param_t, 2> &jtx_param) {
          uint64_t sysref_freq;
          int32_t rc = adi_ad9082_sync_sysref_frequency_set(
              device, &sysref_freq, dev_ref_freq_hz, dac_freq_hz, adc_freq_hz,
              main_interp, ch_interp, cddc_dcm.data(), fddc_dcm.data(),
              jtx_links, jrx_param, jtx_param.data());
          return std::pair<int32_t, uint64_t>(rc, sysref_freq);
        });
  m.def("sync_calc_jrx_lmfc_lemc",
        [](uint64_t dac_clk, uint8_t main_interp, uint8_t ch_interp,
           adi_cms_jesd_param_t *jrx_param) {
          uint64_t jrx_lmfc = 0U;
          int32_t rc = adi_ad9082_sync_calc_jrx_lmfc_lemc(
              dac_clk, main_interp, ch_interp, jrx_param, &jrx_lmfc);
          return std::pair<int32_t, uint64_t>(rc, jrx_lmfc);
        });
  m.def(
      "sync_calc_jtx_lmfc_lemc",
      [](uint64_t adc_clk, std::array<uint8_t, 4> &cddc_dcm,
         std::array<uint8_t, 8> &fddc_dcm, adi_ad9082_jesd_link_select_e links,
         std::array<adi_cms_jesd_param_t, 2> &jtx_param) {
        uint64_t jtx_lmfc = 0U;
        int32_t rc = adi_ad9082_sync_calc_jtx_lmfc_lemc(
            adc_clk, cddc_dcm.data(), fddc_dcm.data(), links, jtx_param.data(),
            &jtx_lmfc);
        return std::pair<int32_t, uint64_t>(rc, jtx_lmfc);
      });
  m.def("sync_sysref_input_config_set",
        adi_ad9082_sync_sysref_input_config_set);
  m.def("jesd_sysref_setup_hold_get", [](adi_ad9082_device_t *device) {
    uint8_t setup_risk_violation = 0, hold_risk_violation = 0;
    int32_t rc = adi_ad9082_jesd_sysref_setup_hold_get(
        device, &setup_risk_violation, &hold_risk_violation);
    return std::tuple<int32_t, uint8_t, uint8_t>(rc, setup_risk_violation,
                                                 hold_risk_violation);
  });
  m.def("jesd_sysref_average_set", adi_ad9082_jesd_sysref_average_set);
  m.def("jesd_sysref_monitor_phase_get", [](adi_ad9082_device_t *device) {
    uint16_t sysref_phase = 0;
    int32_t rc =
        adi_ad9082_jesd_sysref_monitor_phase_get(device, &sysref_phase);
    return std::pair<int32_t, uint16_t>(rc, sysref_phase);
  });
  m.def("jesd_oneshot_sync", adi_ad9082_jesd_oneshot_sync);
}
